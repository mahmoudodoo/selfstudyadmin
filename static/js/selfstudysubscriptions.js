                ...
                render: function(data, type, row) {
                    return `
                        <div class="action-buttons">
                            <button class="btn btn-sm btn-primary" onclick="editFeature(${row.id})">
                                <i class="fas fa-edit"></i> Edit
                            </button>
                            <button class="btn btn-sm btn-danger" onclick="deleteResource('feature', ${row.id})">
        ...
        columns: [
            ...
            { 
                ...
                render: function(data, type, row) {
                    return `
                        <div class="action-buttons">
                            <button class="btn btn-sm btn-primary" onclick="editSubscriptionType(${row.id})">
                                <i class="fas fa-edit"></i> Edit
                            </button>
                            <button class="btn btn-sm btn-danger" onclick="deleteResource('subscription_type', ${row.id})">
                                <i class="fas fa-trash"></i> Delete
                            </button>
                        </div>
                    `;
...
function populateEditForm(data, resourceType) {
    switch(resourceType) {
        ...
        case 'subscription':
            ...
            loadSubscriptionTypesForSelect(data.subscription_type?.id);
            break;
    }
}
...
async function loadFeaturesForSelect(selectedFeatures = []) {
    try {
        ...
        response.forEach(feature => {
            ...
            select.append(`
                <option value="${feature.id}" ${isSelected ? 'selected' : ''}>
                    ${feature.name}
                </option>
            `);
        });
...
function editFeature(id) {
    ...
    $.ajax({
        ...
        const feature = response.find(f => f.id === id);
        if (feature) {
...
function editSubscriptionType(id) {
    ...
    $.ajax({
        ...
        const type = response.find(t => t.id === id);
        if (type) {
...
async function loadUsers() {
    try {
        ...
        response.forEach(user => {
            userList.append(`
                <div class="user-item" data-user-id="${user.user_id}" data-email="${user.email || ''}">
                    <div class="user-info">
                        <div class="user-details">
                            <div class="user-id">${user.user_id}</div>
                            <div class="user-email">${user.email || 'No email'}</div>
                        </div>
                        <button class="btn btn-sm btn-primary user-select-btn" onclick="selectUser('${user.user_id}', '${user.email || ''}')">
                            Select
                        </button>
                    </div>
                </div>
