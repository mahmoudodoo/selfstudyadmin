...
class SelfStudyExamManager {
    ...
    async fetchExamQuestions(examId) {
        ...
        try {
            ...
            if (response.success) {
                ...
                const filteredQuestions = questions.filter(question => question.exam === examId);
    ...
    async fetchQuizQuestions(quizId) {
        ...
        try {
            ...
            if (response.success) {
                ...
                const filteredQuestions = questions.filter(question => question.quiz === quizId);
                if (filteredQuestions.length !== questions.length) {
    ...
    async editExam(externalId) {
        ...
        const option = Array.from(courseSelect.options).find(opt => opt.value === exam.course_id);
        if (option) {
    ...
    async editQuiz(externalId) {
        ...
        const courseOption = Array.from(courseSelect.options).find(opt => opt.value === quiz.course_id);
        if (courseOption) {
    ...
    async deleteExamAppointment(externalId) {
        if (!confirm('Are you sure you want to delete this exam appointment? This action cannot be undone.')) return;
        this.showLoading();
        try {
            const response = await this.apiRequest('POST', {
    ...
    async deleteExamResult(externalId) {
        if (!confirm('Are you sure you want to delete this exam result? This action cannot be undone.')) return;
        this.showLoading();
        try {
            const response = await this.apiRequest('POST', {
    ...
    async deleteQuizResult(externalId) {
        if (!confirm('Are you sure you want to delete this quiz result? This action cannot be undone.')) return;
        this.showLoading();
        try {
            const response = await this.apiRequest('POST', {
    ...
    async deleteExamQuestion(questionId) {
        if (!confirm('Are you sure you want to delete this exam question? This action cannot be undone.')) return;
        this.showLoading();
        try {
            const response = await this.apiRequest('POST', {
    ...
    async deleteExamAnswer(answerId) {
        if (!confirm('Are you sure you want to delete this exam answer? This action cannot be undone.')) return;
        this.showLoading();
        try {
            const response = await this.apiRequest('POST', {
    ...
    async deleteQuizAnswer(answerId) {
        if (!confirm('Are you sure you want to delete this quiz answer? This action cannot be undone.')) return;
        this.showLoading();
        try {
            const response = await this.apiRequest('POST', {
    ...
    openImportQuizModal() {
        ...
        document.getElementById('import-quiz-lesson-select').disabled = true;
        document.getElementById('import-quiz-lesson-id').value = '';
    }
    ...
    async validateExamCourse() {
        const jsonText = document.getElementById('exam-json-text').value.trim();
        if (!jsonText) {
            this.showToast('Please paste JSON first', 'error');
            return;
        }
        let examData;
        try {
        ...
        if (!courseId) {
            this.showToast('No course_id found in JSON', 'error');
            return;
        }
        ...
        try {
            const response = await this.apiRequest('POST', {
                action: 'validate_course',
                course_id: courseId
            });
            if (response.success) {
                if (response.exists) {
                    this.showToast(`✅ Course "${response.course.title}" (${courseId}) exists`, 'success');
                } else {
                    this.showToast(`❌ Course ID "${courseId}" not found`, 'error');
                }
            } else {
    ...
    async validateQuizCourseLesson() {
        const jsonText = document.getElementById('quiz-json-text').value.trim();
        if (!jsonText) {
            this.showToast('Please paste JSON first', 'error');
            return;
        ...
        const lessonId = quizData.lesson_id;
        if (!courseId || !lessonId) {
            this.showToast('course_id and lesson_id are required in JSON', 'error');
            return;
        }
        ...
        try {
            ...
            if (response.success) {
                if (!response.course_exists) {
                    this.showToast(`❌ Course ID "${courseId}" not found`, 'error');
                } else if (!response.lesson_exists) {
                    this.showToast(`❌ Lesson ID "${lessonId}" not found under course "${courseId}"`, 'error');
                } else {
                    this.showToast(`✅ Course "${response.course.title}" and lesson "${response.lesson.title}" are valid`, 'success');
                }
            } else {
                throw new Error(response.error || 'Validation failed');
            }
    ...
    onImportExamCourseSelect() {
        ...
        document.getElementById('import-exam-course-id').value = courseId;
    }

    async onImportQuizCourseSelect() {
        const select = document.getElementById('import-quiz-course-select');
        const courseId = select.value;
        ...
        if (!courseId) {
            lessonSelect.innerHTML = '<option value="">First select a course</option>';
            lessonSelect.disabled = true;
            return;
        }
        ...
        try {
            ...
            lessons.forEach(lesson => {
                const lessonId = lesson.external_lesson_id || lesson.id;
    ...
    onImportQuizLessonSelect() {
        const select = document.getElementById('import-quiz-lesson-select');
        const lessonId = select.value;
        document.getElementById('import-quiz-lesson-id').value = lessonId;
    }

    // ===== Course and Lesson Handling for main forms =====
...
function openCreateExamModal() { examManager.openCreateExamModal(); }
function openCreateQuizModal() { examManager.openCreateQuizModal(); }
function openCreateExamQuestionModal() { examManager.openCreateExamQuestionModal(); }
function openCreateQuizQuestionModal() { examManager.openCreateQuizQuestionModal(); }
function openCreateExamAnswerModal() { examManager.openCreateExamAnswerModal(); }
function openCreateQuizAnswerModal() { examManager.openCreateQuizAnswerModal(); }
function closeExamModal() { examManager.closeExamModal(); }
function closeQuizModal() { examManager.closeQuizModal(); }
function closeExamQuestionsModal() { examManager.closeExamQuestionsModal(); }
function closeQuizQuestionsModal() { examManager.closeQuizQuestionsModal(); }
function closeExamQuestionModal() { examManager.closeExamQuestionModal(); }
function closeQuizQuestionModal() { examManager.closeQuizQuestionModal(); }
function closeExamAnswersModal() { examManager.closeExamAnswersModal(); }
function closeQuizAnswersModal() { examManager.closeQuizAnswersModal(); }
function closeExamAnswerModal() { examManager.closeExamAnswerModal(); }
function closeQuizAnswerModal() { examManager.closeQuizAnswerModal(); }
function closeExamAppointmentModal() { examManager.closeExamAppointmentModal(); }
function closeExamResultModal() { examManager.closeExamResultModal(); }
function closeQuizResultModal() { examManager.closeQuizResultModal(); }

// Import modals
function openImportExamModal() { examManager.openImportExamModal(); }
function closeImportExamModal() { examManager.closeImportExamModal(); }
function handleExamFileUpload(event) { examManager.handleExamFileUpload(event); }
function validateExamJSON() { examManager.validateExamJSON(); }
function submitExamJSON() { examManager.submitExamJSON(); }
function openImportQuizModal() { examManager.openImportQuizModal(); }
function closeImportQuizModal() { examManager.closeImportQuizModal(); }
function handleQuizFileUpload(event) { examManager.handleQuizFileUpload(event); }
function validateQuizJSON() { examManager.validateQuizJSON(); }
function submitQuizJSON() { examManager.submitQuizJSON(); }

// Validation
function validateExamCourse() { examManager.validateExamCourse(); }
function validateQuizCourseLesson() { examManager.validateQuizCourseLesson(); }

// Import selectors
function onImportExamCourseSelect() { examManager.onImportExamCourseSelect(); }
function onImportQuizCourseSelect() { examManager.onImportQuizCourseSelect(); }
function onImportQuizLessonSelect() { examManager.onImportQuizLessonSelect(); }
...
function copyToClipboard(elementId) {
    ...
    navigator.clipboard.writeText(input.value).then(() => {
        examManager.showToast('Copied to clipboard!', 'success');
    }).catch(() => {
        examManager.showToast('Failed to copy', 'error');
    });
}

function onCourseSelect(type) { examManager.onCourseSelect(type); }
function onLessonSelect() { examManager.onLessonSelect(); }

async function refreshData() { await examManager.loadInitialData(); }
